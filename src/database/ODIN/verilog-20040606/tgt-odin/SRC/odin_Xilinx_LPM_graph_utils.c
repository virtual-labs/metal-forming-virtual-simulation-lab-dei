...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_generate_vqm)
 * 	Basic traversal of the new datastructure.
 *-------------------------------------------------------------------------------------------*/
void oXLgu_generate_vqm(int number_to_mark_as_flag, int *cell_count, FILE *out, ivl_design_t des, const char *path)
{
	int i;
	int idx;
    char *module_name;
	...
	oXLgu_depth_first_traversal_start(out);
 ...
 *-------------------------------------------------------------------------------------------*/
void oXLgu_depth_first_traversal_start(FILE *out)
{
	int i;
	...
	for (i = 0; i < num_input_nodes; i++)
	{
		...
		{
			oXLgu_depth_first_traverse_wire_define_pass(input_nodes[i], out, QUARTUS_LPM_TRAVERSE_WIRE);
		}
	}
	/* now traverse the ground and vcc pins */
	oXLgu_depth_first_traverse_wire_define_pass(gnd_node, out, QUARTUS_LPM_TRAVERSE_WIRE);
	oXLgu_depth_first_traverse_wire_define_pass(vcc_node, out, QUARTUS_LPM_TRAVERSE_WIRE);
	...
	for (i = 0; i < num_input_nodes; i++)
	{
		if (input_nodes[i] != NULL)
		{
			oXLgu_depth_first_traverse(input_nodes[i], out, QUARTUS_LPM_TRAVERSE);
		}
	}
	/* now traverse the ground and vcc pins */
	oXLgu_depth_first_traverse(gnd_node, out, QUARTUS_LPM_TRAVERSE);
	oXLgu_depth_first_traverse(vcc_node, out, QUARTUS_LPM_TRAVERSE);
}

/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_depth_first_traverse_wire_define_pass)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_depth_first_traverse_wire_define_pass(node_t *node, FILE *out, int traverse_mark_number)
{
	int i;
	char* temp_string;
	...
	else
	{
		...
		if (node->node_type == MAPPED_NODE) // (node->is_partial_mapped_bound == HPM_HARD_MAPPED)
		{
			...
			oXLgu_generate_hard_mapped_device_wire_pass(node, out);

			/* now propogate forward on all the output pins */
			oXLgu_wire_define_output_traverse_SPECIAL_for_hard_mapped(node, out, traverse_mark_number);
		}
		else if ((node->node_type == LIBRARY_NODE) || (node->node_type == LIBRARY_NODE_FF)) 
		{
			...
			switch(quartus_lib_imp[library_index].definition_style)
			{
				case QL_LOGIC: 
				{
					temp_string = oXLgu_generate_wire_name(node, 0);
					fprintf(out, "wire %s;\n", temp_string);
					ou_free(temp_string);
					break;
				}
				case QL_DEFINITION: 
				{
					...
					{
						temp_string =  oXLgu_generate_wire_name(node, i);
						fprintf(out, "wire %s;\n", temp_string);
						ou_free(temp_string);
					}
					break;
			...
			oXLgu_wire_define_output_traverse(node, out, traverse_mark_number);
		}
		else if (node->node_type == MACRO_NODE)
		{
			/* do the wire assessment for the hetero nodes */
			oXLgu_generate_hetero_device_wire_pass(node, out);
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_wire_define_output_traverse)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_wire_define_output_traverse(node_t *node, FILE *out, int traverse_mark_number)
{
	int i, j;
	...
	for (i = 0; i < node->num_output_pins; i++)
	{
		for (j = 0; j < onu_get_output_pins_related_num_level_2(node->output_pins[i]); j++)
		{
			if(onu_get_output_pins_node(node->output_pins[i], j) != NULL)
			{
				...
				oXLgu_depth_first_traverse_wire_define_pass (onu_get_output_pins_node(node->output_pins[i], j), out, traverse_mark_number);
			}
		}
	}
}
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_wire_define_output_traverse_SPECIAL_for_hard_mapped)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_wire_define_output_traverse_SPECIAL_for_hard_mapped(node_t *node, FILE *out, int traverse_mark_number)
{
	int i, k, l;
	...
	for (i = 0; i < node->binding->tfe_implementation->output_nodes->num_nodes; i++)
	{
		for (k = 0; k < node->binding->tfe_implementation->output_nodes->nodes[i]->num_output_pins; k++)
		{
			for (l = 0; l < onu_get_output_pins_related_num_level_2(node->binding->tfe_implementation->output_nodes->nodes[i]->output_pins[k]); l++)
			{
				...
				{
					/* recursively depth in */
					oXLgu_depth_first_traverse_wire_define_pass (onu_get_output_pins_node(node->binding->tfe_implementation->output_nodes->nodes[i]->output_pins[k], l), out, traverse_mark_number);
				}
#if 0
				if(onu_get_output_pins_node(node->output_pins[k], l) != NULL)
				{
					/* recursively depth in */
					oXLgu_depth_first_traverse_wire_define_pass (onu_get_output_pins_node(node->output_pins[k], l), out, traverse_mark_number);
				}
#endif
			}
		}
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_depth_first_traverse)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_depth_first_traverse(node_t *node, FILE *out, int traverse_mark_number)
{
	int i;
	...
	else
	{
		...
		if (node->node_type == MAPPED_NODE) // (node->is_partial_mapped_bound == HPM_HARD_MAPPED)
		{
			...
			oXLgu_generate_hard_mapped_device(node, node->unique_name, out);

			/* now traverse forward */
			oXLgu_device_define_output_traverse_SPECIAL_for_hard_mapped(node, out, traverse_mark_number);
		}
		else if ((node->node_type == LIBRARY_NODE) || (node->node_type == LIBRARY_NODE_FF))
		{
			/* IF - this is a library node then we can drop the implementation */
			...
			switch(quartus_lib_imp[library_index].definition_style)
			{
				case QL_LOGIC: 
				{
					...
					oXLgu_define_logical_function(
							node, 
							library_index, 
							oXLgu_get_unique_name(node), 
							out);
					break;
				}
				case QL_DEFINITION: 
				{
					oXLgu_define_compound_logic(node,
										oXLgu_get_unique_name(node),
										library_index,
										out);	
					break;
				...
				case QL_DFF: 
				{
					...
					oXLgu_define_register(node, oXLgu_get_unique_name(node), library_index, out);
					break;
				}
			}

			/* once the info has been dropped down, then traverse the additional nodes */
			oXLgu_device_define_output_traverse(node, out, traverse_mark_number);
		}
		else if (node->node_type == MACRO_NODE)
		{
			...
			oXLgu_generate_hetero_device(node, oXLgu_get_unique_name(node), out);	
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_device_define_output_traverse)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_device_define_output_traverse(node_t *node, FILE *out, int traverse_mark_number)
{
	int i, j;
	...
	for (i = 0; i < node->num_output_pins; i++)
	{
		for (j = 0; j < onu_get_output_pins_related_num_level_2(node->output_pins[i]); j++)
		{
			...
			{
				/* recursively depth in */
				oXLgu_depth_first_traverse (onu_get_output_pins_node(node->output_pins[i], j), out, traverse_mark_number);
			}
		}
	}
}
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_device_define_output_traverse_SPECIAL_for_hard_mapped)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_device_define_output_traverse_SPECIAL_for_hard_mapped(node_t *node, FILE *out, int traverse_mark_number)
{
	int i, k, l;
	...
	for (i = 0; i < node->binding->tfe_implementation->output_nodes->num_nodes; i++)
	{
		for (k = 0; k < node->binding->tfe_implementation->output_nodes->nodes[i]->num_output_pins; k++)
		{
			for (l = 0; l < onu_get_output_pins_related_num_level_2(node->binding->tfe_implementation->output_nodes->nodes[i]->output_pins[k]); l++)
			{
				...
				{
					/* recursively depth in */
					oXLgu_depth_first_traverse (onu_get_output_pins_node(node->binding->tfe_implementation->output_nodes->nodes[i]->output_pins[k], l), out, traverse_mark_number);
				}
#if 0
				if(onu_get_output_pins_node(node->output_pins[k], l) != NULL)
				{
					/* recursively depth in */
					oXLgu_depth_first_traverse (onu_get_output_pins_node(node->output_pins[k], l), out, traverse_mark_number);
				}
#endif
			}
		}
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_define_logical_function)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_define_logical_function(
							node_t *node, 
							int library_index, 
							char *unique_name, 
							FILE *out)
{
	...
	output_strings[0] =  oXLgu_generate_wire_name(node, 0);
	...
	{
		wire_string =  oXLgu_generate_wire_name(onu_get_input_pins_node(node->input_pins[i]), 
	...
	oXLgu_create_an_output_joining(node, output_strings, out);
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_define_registe)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_define_register(node_t *node, char *unique_name, int library_index, FILE *out)
{
	char *unique_print_name = ou_strdup(ou_flatten_odin_name(unique_name), HETS_QUARTUS_LPM_GRAPH_UTILS);
	char *wire_string;
	char **output_strings = (char**)ou_malloc(sizeof(char*), HETS_QUARTUS_LPM_GRAPH_UTILS);
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_define_compound_logic)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_define_compound_logic(node_t *node, char *unique_name, int library_index, FILE *out)
{
	int i;
	char *unique_print_name = ou_strdup(ou_flatten_odin_name(unique_name), HETS_QUARTUS_LPM_GRAPH_UTILS);
	char *wire_string;
	...
	for (i = 0; i < quartus_lib_imp[library_index].num_output_ports; i++)
	{
		...
		output_strings[i] =  oXLgu_generate_wire_name(node, i);
		/* printout the output hookup */
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_generate_hard_mapped_device_wire_pass)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_generate_hard_mapped_device_wire_pass(node_t *node, FILE *out)
{
	int i, j, k;
	char *wire_string;
	int output_idx = 0;
	...
	for (i = 0; i < node->binding->tfe_implementation->output_nodes->num_nodes; i++)
	{
		for (j = 0; j < node->binding->tfe_implementation->output_nodes->nodes[i]->num_output_ports; j++)
		{
			...
			{
				wire_string =  oXLgu_generate_wire_name(node, output_idx);
				fprintf(out, "wire %s;\n", wire_string);
				ou_free(wire_string);
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_generate_hetero_device_wire_pass)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_generate_hetero_device_wire_pass(node_t *node, FILE *out)
{
	int i;
	char *wire_string;
	int width;
	...
	switch (node->n_t.oetero_node.oetero_node_type)
	{
		case MN_MEMORY:
		{
			int memory_width = ivl_memory_width(node->n_t.oetero_node.memory);

			/* create the final output connection if the outputs of this function is conneted to a primary output */
			for (i = 0; i < memory_width; i++)
			{
				wire_string =  oXLgu_generate_wire_name(node, i);
				fprintf(out, "wire %s;\n", wire_string);
				ou_free(wire_string);
			}
			break;
		...
		case MN_MULT:
		{
			width = node->n_t.oetero_node.width;
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_generate_hetero_device)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_generate_hetero_device(node_t *node, char *unique_name, FILE *out)
{
	char *wire_string;
	char *unique_print_name = ou_strdup(ou_flatten_odin_name(unique_name), HETS_QUARTUS_LPM_GRAPH_UTILS);
	int i;
	...
	switch (node->n_t.oetero_node.oetero_node_type)
	{
		...
		{
			int memory_width = ivl_memory_width(node->n_t.oetero_node.memory);
			int memory_size = ivl_memory_size(node->n_t.oetero_node.memory);
			int address_width = node->n_t.oetero_node.memory_details->address_width;
			...
			oXLgu_generate_concat_input_port_list(node,  address_width, 2+memory_width, out);
			fprintf(out, "),\n"); 

			/* hookup the rdaddress */
			fprintf(out, "\t.address_b("); 
			oXLgu_generate_concat_input_port_list(node,  address_width, 2+memory_width+address_width, out);
			fprintf(out, "),\n"); 

			/* hook up the data port */
			fprintf(out, "\t.data_a("); 
			oXLgu_generate_concat_input_port_list(node,  memory_width, 2, out); 
			fprintf(out, "),\n"); 

			/* hookup the output port */
			fprintf(out, "\t.q_b("); 
			output_strings = oXLgu_generate_concat_output_port_list(node,  memory_width, 0, out);
			fprintf(out, "));\n"); 

			/* send out the parameters - obtained by creating a MegaFunction in Quartus */
			fprintf(out, "defparam\n\t%s.operation_mode = \"DUAL_PORT\",\n", unique_print_name);
			fprintf(out, "\t%s.width_a = %d,\n", unique_print_name, memory_width);
			fprintf(out, "\t%s.widthad_a = %d,\n", unique_print_name, address_width);
			fprintf(out, "\t%s.width_b = %d,\n", unique_print_name, memory_width);
			fprintf(out, "\t%s.widthad_b = %d,\n", unique_print_name, address_width);
			fprintf(out, "\t%s.numwords_a = %d,\n", unique_print_name, memory_size);
		...
		case MN_MULT:
		{
			int widtha = node->n_t.oetero_node.width_a;
			int widthb = node->n_t.oetero_node.width_b;
			int width = node->n_t.oetero_node.width;
			...
			/* hook up the data port */
			fprintf(out, "\t.dataa("); 
			oXLgu_generate_concat_input_port_list(node,  widtha, 0, out);
			fprintf(out, "),\n"); 

			/* hook up the datab port */
			fprintf(out, "\t.datab("); 
			oXLgu_generate_concat_input_port_list(node, widthb, widtha, out);
			fprintf(out, "),\n"); 

			/* hookup the output port */
			fprintf(out, "\t.result("); 
			output_strings = oXLgu_generate_concat_output_port_list(node,  width, 0, out);
			fprintf(out, "));\n"); 
...
//			fprintf(out, "%s.dedicated_multiplier_circuitry = \"YES\",\n", unique_print_name);
		...
		case MN_SUB:
		{
			...
			if (node->n_t.oetero_node.oetero_node_type == MN_ADD) 
			{
		...
		case MN_ADD_CARRY_NODE:
		{
			...
			/* hook up the data port */
			fprintf(out, "\t.dataa("); 
			oXLgu_generate_concat_input_port_list(node,  width, 0, out);
			fprintf(out, "),\n"); 

			/* hook up the datab port */
			fprintf(out, "\t.datab("); 
			oXLgu_generate_concat_input_port_list(node, width, width, out);
			fprintf(out, "),\n"); 

			/* hook up the cin port - the last input pin */
			wire_string =  oXLgu_generate_wire_name(onu_get_input_pins_node(node->input_pins[2*width]), 
		...
		case MN_COUNTER:
		{
			...
			/* define the parameters - based on MegaFunction in Quartus and help files */
			fprintf(out, "%s.lpm_width = %d,\n", unique_print_name, width);
			if (node->n_t.oetero_node.counter_dir == MN_ADD) 
			{
				/* IF - this is an addition then make the quartus lpm addition */
		...
		case MN_CONST_CASE:
		{
			...
			int i, j;
			int num_cases = node->n_t.oetero_node.num_cases-1; // -1 since the case processing always adds a default which we don't want in the constant
			int input_port_index = node->n_t.oetero_node.muxed_input_start_index;
			int *pre_analyse_NULL_count;
			int width_sel = node->n_t.oetero_node.width;
			...
			input_port_index = node->n_t.oetero_node.muxed_input_start_index;
			...
			for (i = 0; i < width; i++)
			{
				...
				/* hookup select */
				fprintf(out, "\t.sel("); 
				oXLgu_generate_concat_input_port_list(node, width_sel, 0, out);
				fprintf(out, "),\n"); 
		
				assert (num_cases - pre_analyse_NULL_count[i] > 1);
		
				/* create the string list of ordered inputs */
				max_size_mux = node->n_t.oetero_node.case_order[num_cases]+1;
				...
				for (j = 0; j < num_cases; j++)
				{
					assert(node->n_t.oetero_node.case_order[j] < max_size_mux);
					assert(node->n_t.oetero_node.case_order[j] != -1);
					assert(input_ordered_list[node->n_t.oetero_node.case_order[j]] == NULL);
		
					if (node->input_pins[input_port_index]->input_node != null_node)
					{
						/* IF - there is a value for this input port extract it */
						input_ordered_list[node->n_t.oetero_node.case_order[j]] = oXLgu_generate_wire_name(onu_get_input_pins_node(node->input_pins[input_port_index]), 
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_generate_hard_mapped_device)
 *-------------------------------------------------------------------------------------------*/
void oXLgu_generate_hard_mapped_device(node_t *node, char *unique_name, FILE *out)
{
	char *wire_string;
	char *unique_print_name = ou_strdup(ou_flatten_odin_name(unique_name), HETS_QUARTUS_LPM_GRAPH_UTILS);
	int i, j;
	...
	switch (node->binding->match_id)
	{
		...
		case MN_TWO_MULT:
		{
			...
			for (i = 0; i < num_input_nodes; i++)
			{
				for (j = max_input_widtha - 1; j >= 0; j--)
				{
					...
					else
					{
						...
						wire_string =  oXLgu_generate_wire_name(onu_get_input_pins_node(binding->tfe_implementation->input_nodes->nodes[i]->input_pins[j]), 
			...
			fprintf(out, "\t.result("); 
			output_strings = oXLgu_generate_concat_output_port_list(binding->tfe_implementation->output_nodes->nodes[0], 
			...
			/* create the final output connection if the outputs of this function is conneted to a primary output */
			oXLgu_create_an_output_joining(binding->tfe_implementation->output_nodes->nodes[0], output_strings, out);
		...
		case MN_MULT_ADD_PACK:
		{
			...
			for (j = max_input_widtha - 1; j >= 0; j--)
			{
				...
				{
					/* ELSE - define the wire */
					wire_string =  oXLgu_generate_wire_name(onu_get_input_pins_node(binding->tfe_implementation->input_nodes->nodes[0]->input_pins[j]), 
			...
			for (j = max_input_widtha - 1; j >= 0; j--)
			{
				...
				{
					wire_string =  oXLgu_generate_wire_name(onu_get_input_pins_node(binding->tfe_implementation->inout_nodes->nodes[0]->input_pins[j+index_adder_non_multiplier_node]), 
			...
			fprintf(out, "\t.result("); 
			output_strings = oXLgu_generate_concat_output_port_list(binding->tfe_implementation->inout_nodes->nodes[0], 
			...
			/* create the final output connection if the outputs of this function is conneted to a primary output */
			oXLgu_create_an_output_joining(binding->tfe_implementation->inout_nodes->nodes[0], output_strings, out);
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_create_an_output_joining)
 * 	Searches all the outputs to check if there is any connection to a Primary Output.
 *-------------------------------------------------------------------------------------------*/
void oXLgu_create_an_output_joining(node_t *node, char** output_strings, FILE *out)
{
	int i, j;
		...
		for (j = 0; j < onu_get_output_pins_related_num_level_2(node->output_pins[i]); j++)
		{
			if(onu_get_output_pins_node(node->output_pins[i], j) != NULL)
			{
				if (onu_get_output_pins_node(node->output_pins[i], j)->node_type == OUTPUT_NODE)
				{
					...
					fprintf(out, "assign %s = %s;\n", oXLgu_generate_port_string(onu_get_output_pins_node(node->output_pins[i], j)), output_strings[i]);
				}
			}
		}
	}
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_get_unique_name)
 *-------------------------------------------------------------------------------------------*/
char *oXLgu_get_unique_name(node_t *node)
{
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_generate_wire_name)
 *-------------------------------------------------------------------------------------------*/
char *oXLgu_generate_wire_name(node_t *node, int output_index) 
{
	...
	switch(node->node_type)
	{
		...
		case INPUT_NODE: 
		{
			...
			generated_wire_names = ou_strdup(oXLgu_generate_port_string(node), HETS_QUARTUS_LPM_GRAPH_UTILS);
			break;
		}
		case OUTPUT_NODE: 
		{
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_generate_port_string)
 *-------------------------------------------------------------------------------------------*/
char *oXLgu_generate_port_string(node_t *node_with_port)
{
	char *long_line;
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_generate_concat_input_port_list)
 * 	This generates a concated list for inputs starting at the index and going to the list size
 *-------------------------------------------------------------------------------------------*/
void oXLgu_generate_concat_input_port_list(node_t *node,  int size_list, int index_to_start_at, FILE *out)
{
	int i;
	char *wire_string;
...
/*---------------------------------------------------------------------------------------------
 * (function: oXLgu_generate_concat_output_port_list)
 * 	This generates a concated list for outputs starting at the index and going to the list size
 *-------------------------------------------------------------------------------------------*/
char **oXLgu_generate_concat_output_port_list(node_t *node,  int size_list, int index_to_start_at, FILE *out)
{
	int i;
	char *wire_string;
